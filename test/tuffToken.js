...
describe("TuffToken", function () {
  ...
  it('should change account rewards', async () => {
    let isExcluded = await tuffToken.isExcludedFromReward(accounts[2].getAddress());
    expect(isExcluded).to.equal(false, "should be included in rewards");

    await tuffToken.excludeFromReward(accounts[2].getAddress())

    isExcluded = await tuffToken.isExcludedFromReward(accounts[2].getAddress());
    expect(isExcluded).to.equal(true, "should be excluded from rewards");

    await tuffToken.includeInReward(accounts[2].getAddress());
